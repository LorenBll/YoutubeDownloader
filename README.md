...
- Body (JSON object):
	...
	- `videos` (array, optional): non-empty array of video objects for batch download. Each item must include single-download required fields.
- Returns:
	- `202` -> `{ "task_id": "<uuid>", "status": "queued" }`
	- `202` -> `{ "task_id": "<uuid>", "status": "queued", "video_count": <n> }`
	...
	- `400` -> `{ "error": "Invalid folder path." }`
	- `400` -> `{ "error": "Invalid videos payload.", "video_errors": [{ "index": 0, "error": "..." }] }`
	- `400` -> `{ "error": "videos must be a non-empty array." }`
	- `500` -> `{ "error": "Could not start download worker. The server may be under heavy load.", "task_id": "<uuid>" }`
...
- Returns:
	- `200` -> `{ "task_id": "<uuid>", "status": "queued|in_progress" }`
	- `200` ->
	  ```json
	  {
	      "task_id": "<uuid>",
	...
	- `200` ->
	  ...
	  ```
	- `200` -> `{ "task_id": "<uuid>", "status": "failed", "error": "<reason>" }`
